...
import { getSkillsSchema, createSkillSchema, updateSkillSchema, addEvidenceSchema } from './schema';
...
export const createSkill = async (req: Request, res: Response) => {
  try {
    ...
    const result = await skillService.createSkill(userId, parsed.data, meta);
...
export const updateSkill = async (req: Request, res: Response) => {
  try {
    ...
    const meta = getMeta(req);
    const result = await skillService.updateSkill(userId, skillId, parsed.data, meta);
...
export const addEvidence = async (req: Request, res: Response) => {
  try {
    ...
    const skillId = req.params.id;
    const meta = getMeta(req);
    const result = await skillService.addEvidence(userId, skillId, parsed.data, meta);
    return res.status(201).json({ data: result, meta, error: null });
  } catch (e) {
